 ...
 *
 * This file defines the Server class, an Express-based HTTP server
 * for rendering KumaScript macros within HTML documents.
 *
 * The endpoint for rendering documents is a POST to /doc/, with
 * environment variables encoded in request headers.
 *
 * The server also supports these endpoints:
 *
 * - GET /macros/ returns a list of defined macros
 * - GET /revision/ returns the git revision of this version of KumaScript
 * - GET /healthz/ returns a 204 status if the server is running
 * - GET /readiness/ returns 204 if the server is ready for requests
 *
 * @prettier
 */
...
class Server {
    ...
    constructor() {
        ...
        app.use(firelogger({ logger: config.logging ? console.log : null }));
