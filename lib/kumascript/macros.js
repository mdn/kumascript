...
var MacroProcessor = ks_utils.Class(EventEmitter, {
    ...
    initialize: function (options) {
        ...
        if ($this.options.statsd) {
            ...
            _(['timing', 'increment', 'decrement', 'gauge', 'update_stats'])
                .each(function (n) { $this.statsd[n] = function () {}; });
        }
    },
    ...
    process: function (src, api_ctx, process_done) {
        ...
        try { tokens = ks_parser.parse(src); }
            ...
            errors.push(ex);
            $this.emit('error', ex);
            $this.statsd.increment('kumascript.document_parsing_errors');
        ...
        api_ctx.performAutoRequire(function (err) {
            if (err) {
                ...
                errors.push(ex);
                $this.emit('error', ex);
                $this.statsd.increment('kumascript.autorequire_errors');
            }
            ...
            $this.loadTemplates(loader, templates, src, function (tmpl_errors) {
                ...
                $this.evaluateMacros(api_ctx, templates, macros, src, function (macro_errors) {
                    ...
                    $this.emit('end');
                    $this.statsd.timing('kumascript.process_document',
                        (new Date()) - time_start);
    ...
    loadTemplates: function (loader, templates, src, next_cb) {
        ...
        var template_q = async.queue(function (name, q_next) {
            try {
                ...
                loader.get(name, function (err, tmpl) {
                    if (!err) {
                        templates[name] = tmpl;
                        $this.statsd.timing('kumascript.load_template',
                            (new Date()) - time_start);
                        ...
                        errors.push(ex);
                        $this.emit('error', ex);
                        $this.statsd.increment('kumascript.template_loading_errors');
                    }
                    q_next();
                });
            } catch (e) {
        ...
        $this.statsd.increment('kumascript.template_count', names.length);
    },
    ...
    evaluateMacros: function (api_ctx, templates, macros, src, next_cb) {
        ...
        var macro_q = async.queue(function (hash, q_next) {
            ...
            try {
                ...
                var tmout_timer = setTimeout(function () {
                    ...
                    errors.push(ex);
                    $this.emit('error', ex);
                    $this.statsd.increment('kumascript.macro_timeouts');
                ...
                tmpl.execute(tok.args, clone_ctx, function (err, result) {
                    ...
                    if (err) { 
                        ...
                        errors.push(ex);
                        $this.emit('error', ex);
                        $this.statsd.increment('kumascript.macro_execution_errors');
                        return next('{{ ' + tok.name + ' }}');
                    }
                    $this.statsd.timing('kumascript.evaluate_macro',
                        (new Date()) - time_start);
        ...
        $this.statsd.increment('kumascript.macro_count', hashes.length);
    }
