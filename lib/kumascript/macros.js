...
var MacroProcessor = ks_utils.Class(EventEmitter, {
    ...
    measureWorkers: function () {
        // HACK: The hirelings module should probably do this with its
        // own support for statsd.
        var backlog_ct = this.worker_pool.backlog.length;
        var hirelings = this.worker_pool.hirelings;
        var workers_ct = _.keys(hirelings).length;
        var busy_ct = _.chain(hirelings)
            .filter(function (h) { return !!h.job; })
            .value().length;

        this.statsd.gauge('workers.total', workers_ct);
        this.statsd.gauge('workers.busy', busy_ct);
        this.statsd.gauge('workers.backlog', backlog_ct);
    }
