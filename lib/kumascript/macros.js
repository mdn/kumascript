...
var 
    ...
    // FIXME: This fails on {{MoreBraces("}}")}} or {{StillMore("{{}}{{")}}
    MACRO_RE = /\{\{([^\}]+)\}\}/gi,
...
var MacroProcessor = ks_utils.Class({
    ...
    process: function (src, api_ctx, process_done) {
        ...
        XRegExp.iterate(src, MACRO_RE, function (match) {
            ...
            // TODO: Do I really need trim() here? Or is that regexp's job?
            macros.push([
                (''+match[0]).trim(),
                (''+m_parts[1]).trim(),
                args
        ...
        var macro_exec_fn = function (m, fe_next) {
            ...
            // This function is the common exit point. Captures the macro
            // execution result and carries on with the forEach.
            var next = function (val) {
                macro_results[macro] = val;
                fe_next();
            };

            // Try getting and executing the template, trapping any errors
            // along the way.
            $this.options.loader.get(name, function (err, tmpl) {
                // TODO: Need to do something more robust with errors
                if (err) { return next(macro); }
        ...
        var finish_process_fn = function (err) {
            var result = src.replace(MACRO_RE, function (s) {
                // TODO: Need a better fallback mechanism here for when the
                // macro result might be missing. And, why would it be missing?
