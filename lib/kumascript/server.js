...
var Server = ks_utils.Class({
    ...
    initialize: function (options) {
        ...
        $this.macro_processor.startup(function () {
            ...
            var format = ':remote-addr - - [:date] ":method :url ' + 'HTTP/:http-version" :status :res[content-length] ' + '":referrer" ":user-agent" :response-time';
            var stream = {
                write: function(s) {
                    log.info(s.trim(), {
                        source: "server",
    ...
    listen: function (port) {
        ...
        this.app._kumascript_listener = this.app.listen(port);
    },
