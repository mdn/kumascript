 ...
 * ## ResponseCache
 *
 * Caching machinery for Express HTTP responses
 */
module.exports.ResponseCache = ks_utils.Class({
    ...
    cacheResponse: function (req, res, options, response_cb) {
        ...
        if (res.get('vary')) {
            ...
            vary_names.forEach(function (name) {
                cache_key_parts.push(name + ': ' + req.get(name));
            });
        }
        ...
        res.set('X-Cache-Key', cache_key);

        var ua_cc = parseCacheControl(req.get('cache-control') || '');
        var ims = req.get('if-modified-since');

        var opts = {
            if_none_match: req.get('if-none-match'),
    ...
    revalidate: function (cache_key, opts, req, res, err, response_cb) {
        ...
        function cache_header(name, value) {
        ...
        // Monkeypatch to capture the headers.
        var orig_setHeader = res.setHeader;
        res.setHeader = function (name, value) {
            orig_setHeader.apply(this, arguments);
            cache_header(name, value);
        };

        // Monkeypatch to capture any final headers. This is always called,
        // but only once, either explicitly by the user or implicitly by node.
        var orig_writeHead = res.writeHead;
        res.writeHead = function (statusCode, statusMessage, headers) {
            _.pairs(headers).forEach(function(pair) {
                cache_header.apply(null, pair);
            });
            // If there's not a "Last-Modified" header yet, add a default.
            if ((200 == statusCode) && !(_.has(cached_meta, 'last_modified'))) {
        ...
        res.write = function (chunk, encoding) {
            ...
            cached_writes.push(new Buffer(chunk, encoding).toString('utf-8'));
        };

        // The response was successfully sent, so let's cache the result
        // if the status code was 200.
        res.on('finish', function () {
            if (200 == res.statusCode) {
                var cached_body = cached_writes.join('');
                $this.set(cache_key, opts.max_age, cached_headers, cached_body,
                          cached_meta, function () { /* No-op */ });
            }
        });
    ...
    sendCommonHeaders: function (req, res, meta) {
        ...
        res.set('Age', Math.floor((now - meta.last_modified) / 1000));
        res.set('Date', new Date().toUTCString());
        if (meta.last_modified) {
            res.set('Last-Modified', new Date(meta.last_modified).toUTCString());
        }
        if (meta.etag) {
