...
*/
    ...
    itMacro('Creates correct feature labels for bare features', function(
        macro
    ) {
    ...
    itMacro(
        ...
        function(macro) {
            return macro
                .call('api.feature_with_description')
    ...
    itMacro('Creates correct cell content for no support', function(macro) {
        return macro.call('html.no_support').then(function(result) {
            ...
            assert.include(
                Array.from(
                    dom.querySelector('.bc-table tbody tr td:nth-child(4)')
                        .classList
                ),
                'bc-supports-no'
            );
            assert.equal(
                dom.querySelector(
                    '.bc-table tbody tr td:nth-child(4) abbr span'
                ).textContent,
                'No support'
            );
            assert.include(
    ...
    itMacro(
        ...
        function(macro) {
            return macro
                .call('html.unknown_version_support')
                .then(function(result) {
                    let dom = JSDOM.fragment(result);
                    assert.include(
                        Array.from(
                            dom.querySelector(
                                '.bc-table tbody tr td:nth-child(4)'
                            ).classList
                        ),
                        'bc-supports-yes'
                    );
    ...
    itMacro(
        ...
        function(macro) {
            return macro
                .call('html.partial_unknown_support')
                .then(function(result) {
                    let dom = JSDOM.fragment(result);
                    assert.include(
                        Array.from(
    ...
    itMacro(
        ...
        function(macro) {
            return macro
                .call('html.partial_removed_support')
                .then(function(result) {
                    let dom = JSDOM.fragment(result);
                    assert.include(
                        Array.from(
    ...
    itMacro(
        ...
        function(macro) {
            return macro
                .call('alternative_name.feature')
                .then(function(result) {
                    let dom = JSDOM.fragment(result);
                    assert.include(
                        Array.from(
                            dom.querySelector('.bc-table tbody tr td').classList
                        ),
                        'bc-has-history'
                    );
                    assert.include(
        ...
        function(macro) {
            return macro.call('notes.feature').then(function(result) {
                ...
                assert.include(
                    Array.from(
                        dom.querySelector(
                            '.bc-browser-firefox > .bc-icons > abbr > i'
                        ).classList
                    ),
                    'ic-footnote'
                );
    ...
    itMacro('Adds correct classes for browsers to head of table', function(
        ...
        return macro.call('javascript.feature').then(function(result) {
            ...
            assert.equal(
                browserIcons[6].classList.contains(
                    'bc-head-icon-webview_android'
                ),
                true
            );
            assert.equal(
                browserIcons[7].classList.contains(
