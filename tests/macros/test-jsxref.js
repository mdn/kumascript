...
var sinon = require('sinon'),
    ...
    js_ref_slug = 'Web/JavaScript/Reference/',
    js_ref_url = '/en-US/docs/' + js_ref_slug;
chai.use(chaiAsPromised);

describeMacro('jsxref', function () {
    beforeEachMacro(function (macro) {
        // jsxref calls wiki.getPage at least twice
    ...
    itMacro('One argument (simple global object)', function (macro) {
        ...
        var name = 'Date',
            ...
            title = ('Creates a JavaScript Date instance that represents a' +
                     ' single moment in time. Date objects are based on a' +
                     ' time value that is the number of milliseconds since' +
                     ' 1 January, 1970 UTC.'),
            expected = ('<a href="' + glob_url + '" title="' + title + '">' +
                        '<code>' + name + '</code></a>');
        macro.ctx.wiki.getPage.withArgs(ref_url).returns({});
        macro.ctx.wiki.getPage.withArgs(glob_url).returns({
    ...
    itMacro('One argument (method by title)', function (macro) {
        ...
        var name = 'Array.prototype.join()',
            ...
            ref_url = js_ref_url + partial_slug,
            glob_url = js_ref_url + 'Global_Objects/' + partial_slug,
            title = ('The join() method joins all elements of an array (or' +
    ...
    itMacro('Two arguments (method by slug, display name)', function (macro) {
        ...
        var name = 'function statement',
            ...
            ref_url = js_ref_url + partial_slug,
            glob_url = js_ref_url + 'Global_Objects/' + partial_slug,
            title = ('The function declaration defines a function with the' +
        ...
        return assert.eventually.equal(
            macro.call(partial_slug, name), expected);
    });
    ...
    itMacro('Two arguments (non-global by slug, name)', function (macro) {
        ...
        var name = 'yield',
            ...
            ref_url = js_ref_url + partial_slug,
            glob_url = js_ref_url + 'Global_Objects/' + partial_slug,
            title = ('The yield keyword is used to pause and resume a' +
                     ' generator function (function* or legacy generator' + 
                     ' function).'),
            expected = ('<a href="' + ref_url + '" title="' + title + '">' +
                        '<code>' + name + '</code></a>');
        macro.ctx.wiki.getPage.withArgs(ref_url).returns({
            slug: js_ref_slug + partial_slug,
            summary: title
    ...
    itMacro('Three arguments (slug, name, #anchor)', function (macro) {
        ...
        var name = 'array iteration and for...in',
            ...
            ref_url = js_ref_url + partial_slug,
            glob_url = js_ref_url + 'Global_Objects/' + partial_slug,
            title = ('The for...in statement iterates over the enumerable' +
        ...
        return assert.eventually.equal(
            macro.call(partial_slug, name, anchor), expected);
    });
    ...
    itMacro('Three arguments (slug, name, anchor without hash) [ru]', function (macro) {
        ...
        var name = 'Array',
            ...
            title = ('Массив (Array) в JavaScript является глобальным' +
    ...
    itMacro('Four arguments (slug, name, empty anchor, omit code wrap)', function (macro) {
        // Used on:
        ...
        var name = 'function expressions',
            ...
            ref_url = js_ref_url + partial_slug,
            glob_url = js_ref_url + 'Global_Objects/' + partial_slug,
            title = ('The function keyword can be used to define a function' +
        ...
        macro.ctx.wiki.getPage.withArgs(glob_url).returns({});
        return assert.eventually.equal(
            macro.call(partial_slug, name, "", 1), expected);
    });
    ...
    itMacro('Quotes in summary are escaped', function (macro) {
        ...
        var name = 'name',
            ...
            ref_url = js_ref_url + partial_slug,
            glob_url = js_ref_url + 'Global_Objects/' + partial_slug,
            title = ('A Function object\'s read-only name property indicates' +
    ...
    itMacro('Standard title is used on missing document', function (macro) {
        ...
        var name = 'RegExp.prototype.test()',
            ...
            ref_url = js_ref_url + partial_slug,
            glob_url = js_ref_url + 'Global_Objects/' + partial_slug,
            standard = ('The documentation about this has not yet been' +
                        ' written; please consider contributing!'),
            expected = ('<a href="' + glob_url + '" title="' + standard + '">' +
                        '<code>' + name + '</code></a>');
        macro.ctx.wiki.getPage.withArgs(ref_url).returns({});
        macro.ctx.wiki.getPage.withArgs(glob_url).returns({});
